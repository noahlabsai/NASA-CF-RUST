...
pub unsafe fn CF_CFDP_R_ProcessFd(
    ...
    if (*txn).state_data.cached_pos != fd.offset {
        let fret = CF_WrappedLseek((*txn).fd, fd.offset as i32, OS_SEEK_SET);
    ...
    if ret != CF_ERROR {
        let fret = CF_WrappedWrite((*txn).fd, fd.data_ptr as *const c_void, fd.data_len as u32);
        if fret != fd.data_len as i32 {
            ...
            CF_ChunkListAdd(&mut (*(*txn).chunks).chunks, fd.offset, fd.data_len);
        }
    }
...
pub unsafe fn CF_CFDP_R2_GapCompute(
    _chunks: *const CF_ChunkList_t,
    chunk: *const CF_Chunk_t,
    opaque: *mut c_void,
) {
    ...
    if (*pseglist).num_segments < CF_PDU_MAX_SEGMENTS as u32 {
...
pub unsafe fn CF_CFDP_R_SendNak(txn: *mut CF_Transaction_t) -> CFE_Status_t {
    ...
    if (*txn).flags.rx.md_recv {
        ...
        if cret != 0 {
            ...
            CF_AppData.hk.Payload.channel_hk[(*txn).chan_num as usize].counters.sent.nak_segment_requests += cret;
        }
    } else {
        CFE_EVS_SendEvent!(
...
pub unsafe fn CF_CFDP_R_Init(txn: *mut CF_Transaction_t) {
    ...
    (*txn).state_data.fin_dc = CF_CFDP_FinDeliveryCode_t::CF_CFDP_FinDeliveryCode_INVALID;
    (*txn).state_data.fin_fs = CF_CFDP_FinFileStatus_t::CF_CFDP_FinFileStatus_INVALID;
    ...
    let ret = CF_WrappedOpenCreate(
        &mut (*txn).fd,
        temp_name.as_ptr() as *const c_char,
...
pub unsafe fn CF_CFDP_R_CalcCrcStart(txn: *mut CF_Transaction_t) {
    if (*txn).fsize != (*txn).state_data.eof_size {
        ...
        let os_status = CF_WrappedLseek((*txn).fd, 0, OS_SEEK_SET);
        if os_status != 0 {
            CFE_EVS_SendEvent!(
...
pub unsafe fn CF_CFDP_R_CalcCrcChunk(txn: *mut CF_Transaction_t) {
        ...
        && (*txn).state_data.cached_pos < (*txn).fsize
    {
        ...
        let fret = CF_WrappedRead((*txn).fd, buf.as_mut_ptr() as *mut c_void, read_size as u32);
        ...
        CF_CrcDigest(&mut (*txn).crc, buf.as_ptr(), read_size);
...
pub unsafe fn CF_CFDP_R2_SubstateRecvFinAck(
    ...
    if status == CFE_SUCCESS {
        ...
        if ack.ack_directive_code == CF_CFDP_FileDirective_t::CF_CFDP_FileDirective_FIN {
...
pub unsafe fn CF_CFDP_R_HandleFileRetention(txn: *mut CF_Transaction_t) {
    ...
    let mut pending_fs = CF_CFDP_FinFileStatus_t::CF_CFDP_FinFileStatus_INVALID;
    let pending_dc: CF_CFDP_FinDeliveryCode_t;
    ...
    if subject_file.is_null() {
        pending_dc = CF_CFDP_FinDeliveryCode_t::CF_CFDP_FinDeliveryCode_INVALID;
        CF_CFDP_SetTxnStatus(txn, CF_TxnStatus_t::CF_TxnStatus_FILESTORE_REJECTION);
    } else if !(*txn).flags.com.is_complete {
        pending_dc = CF_CFDP_FinDeliveryCode_t::CF_CFDP_FinDeliveryCode_INCOMPLETE;
        CF_CFDP_SetTxnStatus(txn, CF_TxnStatus_t::CF_TxnStatus_INVALID_FILE_STRUCTURE);
    } else {
        pending_dc = CF_CFDP_FinDeliveryCode_t::CF_CFDP_FinDeliveryCode_COMPLETE;
        ...
        if !CF_CFDP_TxnIsOK(txn) || CF_CFDP_R_CheckCrc(txn) != CFE_SUCCESS {
            ...
            if os_status == OS_SUCCESS {
                ...
                pending_fs = CF_CFDP_FinFileStatus_t::CF_CFDP_FinFileStatus_RETAINED;
                CFE_EVS_SendEvent!(
    ...
    if !subject_file.is_null() {
        ...
        if (*(*txn).history).txn_stat == CF_TxnStatus_t::CF_TxnStatus_FILESTORE_REJECTION {
            pending_fs = CF_CFDP_FinFileStatus_t::CF_CFDP_FinFileStatus_DISCARDED_FILESTORE;
        } else {
            pending_fs = CF_CFDP_FinFileStatus_t::CF_CFDP_FinFileStatus_DISCARDED;
        }
    }
...
pub unsafe fn CF_CFDP_R_CheckState_FINACK(txn: *mut CF_Transaction_t) -> CF_RxSubState_t {
    let next_state = (*txn).state_data.sub_state;
...
pub unsafe fn CF_CFDP_R_CheckState(txn: *mut CF_Transaction_t) {
    ...
    if sub < CF_RxSubState_NUM_STATES as usize {
        (*txn).state_data.sub_state = STATE_FNS[sub](txn);
    }
}
