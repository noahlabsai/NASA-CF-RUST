...
pub unsafe fn CF_CFDP_S_SendFileData(
    ...
    if ph.is_null() {
        ...
        if (*txn).state_data.cached_pos != foffs {
            status = CF_WrappedLseek((*txn).fd, foffs as i32, OS_SEEK_SET);
            if status < 0 {
                CFE_EVS_SendEvent!(
                ...
                CF_AppData += 1; // was ++.hk.Payload.channel_hk[(*txn).chan_num as usize].counters.fault.file_seek;
                ret = CF_ERROR;
            }
        }

        if ret == CFE_SUCCESS {
            status = CF_WrappedRead((*txn).fd, fd.data_ptr, bytes_to_read);
            if status != bytes_to_read as i32 {
                ...
                CF_AppData += 1; // was ++.hk.Payload.channel_hk[(*txn).chan_num as usize].counters.fault.file_read;
                ret = CF_ERROR;
            } else {
                actual_bytes = status;
                fd.data_len = actual_bytes as u32;
        ...
        if ret == CFE_SUCCESS {
            ...
            if calc_crc != 0 {
                CF_CRC_Digest(&mut (*txn).crc, fd.data_ptr, fd.data_len);
            }
...
pub unsafe fn CF_CFDP_S2_SubstateNak(
    ...
    if CF_CFDP_RecvNak(txn, ph) == CFE_SUCCESS {
        ...
        CF_AppData.hk.Payload.channel_hk[(*txn).chan_num as usize].counters.recv.nak_segment_requests +=
            nak.segment_list.num_segments as u64;
        if bad_sr != 0 {
            CFE_EVS_SendEvent!(
        ...
        CF_AppData += 1; // was ++.hk.Payload.channel_hk[(*txn).chan_num as usize].counters.recv.error;
    }
}
...
pub unsafe fn CF_CFDP_S2_SubstateEofAck(
    ...
    if ret == CFE_SUCCESS {
        ...
        if ack.ack_directive_code == CF_CFDP_FileDirective_t::CF_CFDP_FileDirective_EOF {
...
pub unsafe fn CF_CFDP_S1_Recv(
    ...
    static S1_SUBSTATE: [*const CF_CFDP_FileDirectiveDispatchTable_t; CF_TxSubState_NUM_STATES] = {
    ...
    CF_CFDP_TxStateDispatch(txn, &S1_SUBSTATE, ph);
}
...
pub unsafe fn CF_CFDP_S2_Recv(
    ...
    static S2_SUBSTATE: [*const CF_CFDP_FileDirectiveDispatchTable_t; CF_TxSubState_NUM_STATES] = {
        let mut a = [ptr::null(); CF_TxSubState_NUM_STATES];
    ...
    CF_CFDP_TxStateDispatch(txn, &S2_SUBSTATE, ph);
}
...
pub unsafe fn CF_CFDP_S_Init(txn: *mut CF_Transaction_t) {
    ...
    if (*(*txn).history).fnames.src_filename[0] == 0 {
        ...
        CF_AppData += 1; // was ++.hk.Payload.channel_hk[(*txn).chan_num as usize].counters.fault.file_open;
...
pub unsafe fn CF_CFDP_S_HandleFileRetention(txn: *mut CF_Transaction_t) {
    ...
    if !CF_CFDP_TxnIsOK(txn) || !(*txn).flags.com.is_complete {
        ...
        if (*txn).reliable_mode == 0 {
            allow_local_remove = true;
        } else {
            allow_local_remove = (*txn).state_data.fin_fs == CF_CFDP_FinFileStatus_t::CF_CFDP_FinFileStatus_RETAINED
                && (*txn).state_data.fin_dc == CF_CFDP_FinDeliveryCode_t::CF_CFDP_FinDeliveryCode_COMPLETE;
        }
...
pub unsafe fn CF_CFDP_S_AckTimerTick(txn: *mut CF_Transaction_t) {
    if (*txn).reliable_mode == 0 || !(*txn).flags.com.ack_timer_armed { return; }
    if !CF_Timer_Expired(&mut (*txn).ack_timer) {
...
unsafe fn CF_CFDP_S1_CheckState_DATA_EOF(txn: *mut CF_Transaction_t) -> CF_TxSubState_t {
    let mut next_state = (*txn).state_data.sub_state;
    if !(*txn).flags.com.close_req {
        (*txn).state_data.fin_dc = CF_CFDP_FinFileStatus_t::CF_CFDP_FinFileStatus_UNREPORTED;
...
unsafe fn CF_CFDP_S_CheckState_DATA_EOF(txn: *mut CF_Transaction_t) -> CF_TxSubState_t {
    if !CF_CFDP_TxnIsOK(txn) {
        ...
        (*txn).state_data.sub_state /* do nothing */
    } else if (*txn).reliable_mode == 0 {
...
pub unsafe fn CF_CFDP_S_CheckState(txn: *mut CF_Transaction_t) {
    let next_state = match (*txn).state_data.sub_state {
...
pub unsafe fn CF_CFDP_S_Tick(txn: *mut CF_Transaction_t) {
    ...
    if !(*txn).flags.com.inactivity_fired
        && (*txn).state_data.sub_state != CF_TxSubState_t::CF_TxSubState_DATA_NORMAL
    {
        if !CF_Timer_Expired(&mut (*txn).inactivity_timer) {
            ...
            if curr_status == CF_CFDP_AckTxnStatus_t::CF_CFDP_AckTxnStatus_ACTIVE {
                ...
                CF_AppData += 1; // was ++.hk.Payload.channel_hk[(*txn).chan_num as usize].counters.fault.inactivity_timer;
...
pub unsafe fn CF_CFDP_S_Tick_Maintenance(txn: *mut CF_Transaction_t) {
    ...
    } else if (*txn).reliable_mode != 0 && (*txn).flags.tx.fin_ack_count != (*txn).flags.tx.fin_count {
...
pub unsafe fn CF_CFDP_S_Tick_Nak(txn: *mut CF_Transaction_t) {
    if (*txn).flags.tx.fd_nak_pending {
        ...
        if chunk.is_null() {
            ...
            let sret = CF_CFDP_S_SendFileData(txn, (*chunk).offset, (*chunk).size, 0);
            if sret > 0 {
