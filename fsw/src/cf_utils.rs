...
pub unsafe fn CF_GetChunkListHead(
    ...
    if direction == CF_Direction_t::CF_Direction_RX as u8 {
...
pub unsafe fn CF_FindUnusedTransaction(
    ...
    CF_CList_Remove_Ex(chan, unsafe { core::mem::transmute::<u8, CF_QueueIdx_t>(CF_QueueIdx_FREE) }, &mut (*txn).cl_node);
    ...
    CF_CList_Remove_Ex(chan, unsafe { core::mem::transmute::<u8, CF_QueueIdx_t>(q_index as u8) }, &mut (*(*txn).history).cl_node);

    (*txn).state = CF_TxnState_t::CF_TxnState_INIT;
...
pub unsafe fn CF_FindTransactionBySequenceNumber(
    ...
    for &q in queues.iter() {
        crate::cf_clist::CF_CList_Traverse(
            (*chan).qs[q as usize],
            core::mem::transmute::<_, CF_CListFn_t>(CF_FindTransactionBySequenceNumber_Impl as unsafe fn(*mut CF_CListNode_t, *mut c_void) -> i32),
...
pub unsafe fn CF_WriteHistoryEntryToFile(
    ...
    let dir_str: &[u8] = if (*history).dir == CF_Direction_t::CF_Direction_RX { b"RX" } else { b"TX" };
    ...
    for i in 0..3 {
        ...
        if linebuf.len() > 0 {
            let wret = CF_WrappedWrite(fd, linebuf.as_ptr(), linebuf.len());
            if wret != linebuf.len() as i32 {
                return CF_ERROR;
            }
        }
    }
